...
First get_claim. Then lookup_policy. If policy_trigger exists, escalate immediately, no coverage pricing. Duplicate also escalates. Otherwise check_coverage for each unique line code and hospital (independent in same turn). Only then get_preauthorisation for codes requiring it and not excluded. Never repeat identical calls. You may batch only calls whose prerequisites were returned in EARLIER turns. Write alone.
