...
from .output_contract import REVISION, validate_schema
...
def code_check(record, expected):
    ...
    try:
        ...
        check(False, 'output schema: ' + str(exc))
    ...
    if expected['decision'] == 'escalate':
        ...
        # Reserve exact matches first, then match identity independently of outcome.
        # One wrong status must not fabricate wrong code/amount diagnostics.
        fields = ('code', 'amount', 'status', 'exclusion', 'preauth')
        unmatched = list(range(len(lines)))
        matches = {}
        predicates = [
            lambda got, want: all(got.get(k) == want[k] for k in fields if k in want),
            lambda got, want: got.get('code') == want['code'] and got.get('amount') == want['amount'],
            lambda got, want: got.get('code') == want['code'],
        ]
        for predicate in predicates:
            for index, line in enumerate(expected['lines']):
                if index in matches:
                    continue
                match = next((i for i in unmatched if isinstance(lines[i], dict) and predicate(lines[i], line)), None)
                if match is not None:
                    matches[index] = lines[match]
                    unmatched.remove(match)
        for index, line in enumerate(expected['lines']):
            got = matches.get(index)
            if got is None:
                check(False, f"line[{index}] {line['code']}: missing line occurrence")
            else:
                for field in fields:
                    if field in line:
                        check(got.get(field) == line[field], f"line[{index}] {line['code']}: wrong {field}")
...
def summarize(rows):
    result = {'trials': len(rows), 'cases': len({x['case_id'] for x in rows}),
              ...
              'judgement_not_reviewable': sum(x['judgement'] == 'not_reviewable' for x in rows),
...
def run_battery(output_dir, *, backend='scripted', version='v2', model=config.MODEL,
    ...
    rubric_path = Path(__file__).with_name('rubric_clarifications.json')
    rubric = read(rubric_path)
    parent = Path(output_dir)
    parent.mkdir(parents=True, exist_ok=True)
    suite = Path(tempfile.mkdtemp(prefix='suite-', dir=parent)).resolve()
    metadata = {'contract_revision': REVISION, 'rubric_sha256': sha(rubric_path), 'backend': backend, 'prompt_version': version, 'model': model if backend == 'live' else 'scripted',
    ...
    write(suite / 'grading_contract.json', {'labels': labels, 'audit': expected, 'clarifications': rubric})
    rows, reviews, pages = [], [], []
    for label in labels:
        cid = label['case_id']
            ...
            if row['judgement'] == 'pending':
                ...
                for index, criterion in enumerate(label['must_record'], 1):
                    reviews.append({'review_id': f'{cid}/{trial}/{index}', 'case_id': cid, 'trial': trial,
                                    'criterion': rubric['replacements'].get(criterion, criterion), 'verdict': '', 'graded_by': '', 'comments': '', 'record_path': record_path})
