...
def main():
    ...
    if args.action == 'preflight':
        ...
        selected = ['CLM-8842','CLM-8888','CLM-8910','CLM-8925','CLM-8952','CLM-16404']
        records = [run(a, cid, folder) for cid in selected]
        spent = sum(r['cost_usd'] for r in records)
        ordinary_cost = max(r['cost_usd'] for r in records if expected[r['case_id']]['decision']=='approve_in_principle')
        negative_cost = max(r['cost_usd'] for r in records if expected[r['case_id']]['decision']!='approve_in_principle')
        ...
        report = dict(assignment=a, package_sha256=digest,
                      ...
                      observed_cost_usd=spent, projected_agent_total_usd=estimate, contract_revision=a['contract_revision'],
