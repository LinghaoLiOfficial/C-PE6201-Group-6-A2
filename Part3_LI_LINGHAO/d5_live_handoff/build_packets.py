...
for a in assignments:
 ...
 for src in sorted((PART/'integration').rglob('*')):
...
with ZipFile(PART/'D5_LIVE_HANDOFF_v1.0.zip','w',ZIP_DEFLATED) as z:
 for f in sorted(ROOT.iterdir()):
  if f.is_file() and f.suffix in {'.md','.json','.csv','.py'}:
   z.write(f,Path('D5_LIVE_HANDOFF')/f.name)
 for f in sorted(packdir.glob('*.zip')):
  z.write(f,Path('D5_LIVE_HANDOFF/packets')/f.name)
