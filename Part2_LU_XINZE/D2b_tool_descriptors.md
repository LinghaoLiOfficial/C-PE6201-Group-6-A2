...
### 3. `lookup_policy(policy_id: str) -> PolicyTerms | PolicyNotFound`

- **WHAT:** Return the policy terms needed to determine status, coverage dates,
  annual-limit headroom and exclusions.
- **INPUT:** `policy_id` is a non-empty policy identifier; an unknown value
  returns `PolicyNotFound`.
- **RETURNS:** One policy record: status, start/end dates, annual limit,
  used-to-date, and at most 2 exclusion rules in the supplied fixture; at most
  8 scalar fields and 120 tokens.
...
| `lookup_policy` returned raw annual-limit values; the model calculated headroom and compared the claim total. | `lookup_policy_with_limit(policy_id, date_of_service, claim_total)` returns `remaining_annual_limit` and `annual_limit_status`. | Approving an annual-limit-exceeded claim because the model performed the subtraction or comparison incorrectly. |
